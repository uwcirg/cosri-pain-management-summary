...
upload(
  ...
  path.join(__dirname, "dstu2_test_patients")
).then(() => {
  // R4
  console.log();
  upload(
    ...
    path.join(__dirname, "r4_test_patients")
  );
});
...
function stringResult(name, responseOrError) {
  ...
  if (
    responseOrError.status == null &&
    responseOrError.response != null &&
    responseOrError.response.status != null
  ) {
...
function upload(release, baseURL, patientPath) {
  ...
  for (const fileName of fs.readdirSync(patientPath)) {
    ...
    if (json.resourceType !== "Bundle") {
      console.error(
        "Test patients must be a valid FHIR Bundle.  Not a Bundle:",
        file
      );
      continue;
    }
    ...
    if (DUMP_TEST_PATIENT_JSON) {
      ...
      const patientFile = path.join(
        dumpFolder,
        fileName.replace(".json", "_patient.json")
      );
      fs.writeFileSync(
        patientFile,
      ...
      const txEntriesFile = path.join(
        dumpFolder,
        fileName.replace(".json", "_entries.json")
      );
      fs.writeFileSync(
        txEntriesFile,
        JSON.stringify(txEntries, null, 2),
        "utf8"
      );
    }

...
export default function OpioidOverdoseAlert({ summaryData }) {
  ...
  const lastIndexOfGoodMME = isEmptyArray(summaryData)
    ? -1
    : summaryData.findIndex((item) => item.MMEValue < 50);
  const lastIndexOfBadMME = isEmptyArray(summaryData)
    ? -1
    : summaryData.findIndex((item) => item.MMEValue >= 50);
  const [contextState, contextStateDispatch] = useReducer(contextReducer, {
    display:
      ...
      lastIndexOfGoodMME <= lastIndexOfBadMME,
    viewedDate: null,
    // loading: true,
    loading: false,
  ...
  return (
    ...
    >
      <strong className="title flex flex-start flex-align-start">
        ...
        This patient's MME has recently increased to 50 or more, they are at
        higher risk of overdose, please review that patient has naloxone
      </strong>
      {/* <ChevronDownIcon
      ...
      {!contextState.viewedDate && !contextState.savingInProgress && (

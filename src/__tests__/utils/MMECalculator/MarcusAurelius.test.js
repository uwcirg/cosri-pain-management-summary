 ...
 * using MS Contin 30 MG Oral Tablet
 */
  ...
  describe("MME Calculations", () => {
    it("should calculate correct MME for Fentanyl Transdermal System, 0.1 mg/hr (10 over 30 days)", () => {
     ...
     // expect(mmeResults).toHaveLength(1);
      expect(mmeResults[0].dosesPerDay.toFixed(2)).toBe("0.33");
    ...
    // it("should calculate correct MME for Fentanyl 25mcg/hr patch", () => {
    //   const medicationRequest = {
    //     medication: {
    //       coding: [
    //         {
    ...
    //     status: { value: "active" },
    //     dispenseRequest: {
    //       quantity: { value: { value: 10 } }, // 10 patches
    //       expectedSupplyDuration: { value: { value: 30 } }, // 30 days
    //     },
    //   };

    //   const mmeResults = MMECalculator.mme([medicationRequest]);

    //   expect(mmeResults[0].dosesPerDay).toBeCloseTo(0.333, 2); // 10 patches / 30 days
    //   expect(mmeResults[0].conversionFactor).toBe(7.2); // Fentanyl patch conversion
    //   // 0.333 patches/day * 25 mcg/hr * 7.2 = ~60 MME/day
    //   expect(mmeResults[0].mme).toBeCloseTo(60, 0);
    // });
  });
